...
2. In Cloudflare, connect the GitHub repo to a Worker build, or deploy with:
   `npx wrangler deploy`
3. If the build log says a KV namespace already exists, that is fine. The config intentionally does not auto-create KV.
4. After the Worker has a script, add bindings/secrets in the dashboard:
   - KV binding: `WATCHLIST`
